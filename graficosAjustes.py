...
def ajuste_x(t):
    return 2.65*(t+0.2)+2.47-0.35*np.sin(8.46*(t+0.2) + 6.06)

def ajuste_y(t):
    return 0.28*np.sin(9*(t-0.12)-2.5)+4.45

def ajuste_vx(t):
    return 4.3*np.exp(-0.3*t)*np.sin(9.024*t + 0.024)+3.5

def ajuste_vy(t):
    return 3.5*np.exp(-0.12*t)*np.sin(9.024*t - 2)+0.1
